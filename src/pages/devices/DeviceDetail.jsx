...
import { ArrowLeft, Battery, Wifi, Trash2 } from 'lucide-react';
        ...
        <div className="page-container max-w-lg mx-auto space-y-6">
            ...
            {readings.length > 0 && (
                <Card title="Recent Readings">
                    {readings.map((r, i) => (
                        <div key={i} className="flex justify-between py-1 text-sm"><span>{formatDate(r.timestamp, 'time')}</span><span>{formatTemperature(r.temperature)}</span><span>{r.humidity}%</span><span>{r.soilMoisture}%</span></div>
                    ))}
                </Card>
            )}
        </div>
