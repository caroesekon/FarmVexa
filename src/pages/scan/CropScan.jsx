...
import { Camera, Upload, X, ImagePlus, Check, History } from 'lucide-react';
...
export default function CropScan() {
    ...
    const startCamera = async () => {
        setCameraError(''); setFile(null); setPreview(null);
        ...
        try {
            ...
            setCameraOn(true);
            setTimeout(() => { if (videoRef.current && streamRef.current) { videoRef.current.srcObject = streamRef.current; videoRef.current.play().catch(() => {}); } }, 100);
        } catch (err) {
    ...
    const capturePhoto = () => {
        ...
        canvas.toBlob((blob) => {
            ...
            setFile(f); setPreview(URL.createObjectURL(blob)); stopCamera();
        }, 'image/jpeg', 0.9);
    };
    ...
    const handleUpload = async () => {
        ...
        formData.append('cropImage', file); formData.append('fieldId', fieldId); formData.append('cropType', cropType);
        try {
                ...
                <div className="space-y-4">
                    ...
                    <Select label="Crop Type" value={cropType} onChange={(e) => setCropType(e.target.value)}
                        options={[
                            ...
                            { value: 'tea', label: 'Tea' }, { value: 'wheat', label: 'Wheat' }, { value: 'rice', label: 'Rice' },
                        ]} />
                </div>
            </Card>
                ...
                {cameraOn ? (
                    <div className="space-y-3">
                        <div className="relative rounded-xl overflow-hidden bg-black">
                            ...
                            <button onClick={stopCamera} className="absolute top-2 right-2 p-2 bg-black/50 hover:bg-black/70 text-white rounded-full"><X className="w-4 h-4" /></button>
                        </div>
                    ...
                    <div className="space-y-3">
                        <div className="relative rounded-xl overflow-hidden">
                            ...
                            <button onClick={clearFile} className="absolute top-2 right-2 p-2 bg-black/50 hover:bg-black/70 text-white rounded-full"><X className="w-4 h-4" /></button>
                        </div>
                        <div className="flex items-center justify-between text-sm text-gray-500"><span className="truncate">{file?.name}</span><span>{(file?.size / 1024 / 1024).toFixed(1)} MB</span></div>
                        <div className="flex gap-2"><Button variant="outline" onClick={clearFile} className="flex-1">Remove</Button><Button variant="outline" onClick={() => fileRef.current?.click()} className="flex-1">Change</Button></div>
                    </div>
                ) : (
                    <div className="space-y-3">
                        <div onClick={() => fileRef.current?.click()} className="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl p-10 text-center cursor-pointer hover:border-primary-500 transition-colors">
                            <ImagePlus className="w-12 h-12 mx-auto text-gray-400 mb-3" />
                            <p className="text-gray-500 font-medium">Tap to select image</p>
                            <p className="text-sm text-gray-400 mt-1">JPG, PNG or WEBP (max 10MB)</p>
                        </div>
                        <div className="flex gap-3">
                            <Button variant="outline" onClick={() => fileRef.current?.click()} className="flex-1"><Upload className="w-4 h-4" /> Gallery</Button>
                            <Button variant="outline" onClick={startCamera} className="flex-1"><Camera className="w-4 h-4" /> Camera</Button>
                        </div>
